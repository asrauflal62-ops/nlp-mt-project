...
def _build_rope_cache(seq_len: int, head_dim: int, device: torch.device, base: float = 10000.0):
    # returns cos, sin: (seq_len, head_dim//2)
    inv_freq = 1.0 / (base ** (torch.arange(0, head_dim, 2, device=device).float() / head_dim))
    t = torch.arange(seq_len, device=device).float()
    freqs = torch.einsum("i,j->ij", t, inv_freq)  # (L, head_dim//2)
...
def _apply_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    ...
    out1 = x1 * cos - x2 * sin
    out2 = x1 * sin + x2 * cos
    out = torch.stack([out1, out2], dim=-1).flatten(-2)  # interleave
    return out
...
class MultiheadSelfAttentionRoPE(nn.Module):
    ...
    def forward(
        ...
        attn_mask: Optional[torch.Tensor],  # (T,T) bool, True=masked
        key_padding_mask: Optional[torch.Tensor],  # (B,T) bool, True=pad
    ) -> torch.Tensor:
        B, T, D = x.shape
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask: Optional[torch.Tensor]):
        h = self.norm1(x)
        attn_out = self.self_attn(h, attn_mask=None, key_padding_mask=src_key_padding_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, y, memory, tgt_mask, tgt_key_padding_mask, memory_key_padding_mask):
        h = self.norm1(y)
        self_out = self.self_attn(h, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask)
...
class TransformerNMT(nn.Module):
    """
    Relative position (RoPE) + LayerNorm.
    No absolute positional encoding is added to embeddings.
    """
    def __init__(self, cfg: TransformerConfig):
        super().__init__()
        self.cfg = cfg
